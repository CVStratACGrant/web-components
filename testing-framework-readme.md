...
```
Legends are used by the automation to treat each field appropriately. Grouped multi-fields (e.g., `multiText`, `multiOption`) have a `triggerSelector` that must be clicked the appropriate amount of times to reveal all the necessary fields of that type. Several multi-field types can map to the same trigger selector which is derived from the form. Conditional fields (e.g., `conditionalOption`,  `conditionalText`) are triggered by another field in the form and are thus all grouped together and run after the unconditional fields. Notice in the above example how there are two legends for the same bill estimator. This is necessary when a form's logic allows the same input to be associated with more than one field type. For instance, `inland-empire-utilities-agency-charges` can be either `multiText` or `text` depending on whether the bill estimator is set as single-family residential. Legends allow for tests to be agnostic of test object field order. So long as the legend is accurate, developers can order the fields however they want. 
---
